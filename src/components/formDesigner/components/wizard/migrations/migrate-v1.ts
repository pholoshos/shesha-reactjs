...
export const migrateV0toV1 = (props: IWizardComponentPropsV0, context: SettingsMigrationContext): IWizardComponentProps => {
    ...
    const steps = tabs?.map<IWizardStepProps>(tab => {
        ...
        const step: IWizardStepProps = {
            ...restTabProps,
            cancelButtonActionConfiguration: getActionConfig(cancelButtonAction, cancelButtonActionScript, cancelEventName, cancelCustomEventNameToDispatch, cancelUniqueStateId, context),
            nextButtonActionConfiguration: getActionConfig(nextButtonAction, nextButtonActionScript, nextEventName, nextCustomEventNameToDispatch, nextUniqueStateId, context),
            backButtonActionConfiguration: getActionConfig(backButtonAction, backButtonActionScript, backEventName, backCustomEventNameToDispatch, backUniqueStateId, context),
            doneButtonActionConfiguration: getActionConfig(doneButtonAction, doneButtonActionScript, doneEventName, doneCustomEventNameToDispatch, doneUniqueStateId, context),
        };
        return step;
    });
...
const getActionConfig = (action: ButtonActionTypeV0, actionScript: string, eventName: string, customEventName: string, uniqueStateId: string, context: SettingsMigrationContext): IConfigurableActionConfiguration => {
    if (!action)
        return undefined;

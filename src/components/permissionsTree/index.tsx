...
import { PermissionDto, usePermissionGetAllTree, usePermissionUpdateParent, usePermissionDelete } from "../../apis/permission";
...
export const PermissionsTree: FC<IPermissionsTreeProps> = ({value, onChange, ...rest}) => {
            ...
            const f = (list: PermissionDto[]) => {
                list.forEach((item) => {
                    ...
                    if (item.child?.length > 0 && Boolean(i)) {
                        if (!Boolean(exp.find(e => { return e == item.id })))
                            exp.push(item.id)
                        f(item.child);
                    }
                });
    ...
    const onCheck = (keys: any, _info: ICheckInfo) => {
        ...
        if (Boolean(onChange))
            onChange(keys?.checked.map((item) => { return item.toString(); }));
    };
    
    const onExpand = (keys: Key[]) => {
        setExpanded(keys);
        setOpenedKeys(keys.map((item) => { return item.toString()}));
    }
    ...
    const onSelect = (keys: Key[]) => {
        ...
        const ids = keys.map((item) => { return item.toString()});
    ...
    const expandParent = (item) => { 
        if (Boolean(item.parentName)) {
            ...
            if (parent != null) {
                if (expanded.find(x => { return x == parent.id; }) == null) {
                    setExpanded(prev => { return [...prev, parent.id]; } );
                }
                expandParent(parent);
            }
        }
    ...
    useEffect(() => {
        if (doCreateRoot) {
            ...
            if (s == null) {
                setAllItems(prev => { addPermission(null, prev);  return [...prev]; });
            } else {
    ...
    const onDrop = (info) => {
        ...
        if (dragItem.parentName == dropItem.name
            || !Boolean(dragItem.parentName) && info.dropToGap)
            return;
            ...
            title: (
                Boolean(item.description) && 
                    <Tooltip title={item.description} mouseEnterDelay={1}>
                        <span>{!item.isDbPermission ? <Tag>sys</Tag> : null} {(item.displayName ?? item.name)}</span>
                    </Tooltip>
                ||
                !Boolean(item.description) && <span>{!item.isDbPermission ? <Tag>sys</Tag> : null} {(item.displayName ?? item.name)}</span>
        ),
    ...
    return (
        <Space direction="vertical" style={{width: "100%"}}>
            <Spin spinning={isFetchingData || isDeleting || isParentUpdating} tip={getLoadingHint()} indicator={<LoadingOutlined style={{ fontSize: 40 }} spin />}>

...
const DynamicPage: PageWithLayout<IDynamicPageProps> = props => {
  ...
  useEffect(() => {
    if (formMarkup?.formSettings?.modelType)
      fetchMeta({ modelType: formMarkup?.formSettings?.modelType }).then(meta => {
        setMetadata(meta);
      });
  }, [formMarkup?.formSettings?.modelType])
  ...
  const gqlFields = useMemo(() => {
    if (!metadata || !formMarkup?.components) return null;
    let fields: IFieldData[] = [];
    let fieldNames = formMarkup?.components.map(i => i.name);
    
    fieldNames = fieldNames.concat(formMarkup?.formSettings?.fieldsToFetch ?? []);

    formMarkup?.components.forEach((item) => {
    ...
    return fields;
  }, [metadata, formMarkup?.components]);
  ...
  }, [id, fetchFields, formMarkup?.formSettings?.getUrl, entityPathId, fetchDataPath]);

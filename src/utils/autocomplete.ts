...
export const useEntityAutocomplete = (props: IAutocompleteProps): AutocompleteReturn => {
    ...
    const valueFetcher = useGet<IAbpWrappedGetEntityListResponse, any, IAutocompletePayload>(`${GENERIC_ENTITIES_ENDPOINT}/GetAll`, {
        lazy: !props.value,
        queryParams: getValuePayload
    });

    const listFetcher = useGet<IAbpWrappedGetEntityListResponse, any, IAutocompletePayload>(`${GENERIC_ENTITIES_ENDPOINT}/GetAll`, {
    ...
    const allItems = useMemo(() => {
        ...
        if (valueItems){
            valueItems.forEach(i => {
                if (result.findIndex(fi => fi.id === i.id) === -1)
                    result.push(i);
            });
        }
